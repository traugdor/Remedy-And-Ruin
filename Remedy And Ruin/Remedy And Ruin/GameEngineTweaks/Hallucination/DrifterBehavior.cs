...
{
    /// <summary>
    /// Drifter's AI flow, driven through ClientEntityAILib's ClientControlledEntity: real
    /// obstacle-avoiding pathfinding for both the wander legs and the beeline (both via the
    /// MoveToSlow/MoveToFast callback overloads), real per-entity-derived speeds, and
    /// terrain-following, all handled by the library. This class only owns the AI decisions:
    /// which node to wander to next, how long to pause, when to switch to beelining, and when
    /// close enough to the player to attack. Kept fully separate from ShiverBehavior (rather than
    /// sharing a base class) so drifter-only additions don't touch shiver's flow at all.
    /// </summary>
    internal class DrifterBehavior : IApparitionBehavior
    {
        ...
        // Matches attackDurationMs: 1000 on the real drifter's meleeattack AI task
        // (assets/survival/entities/lore/drifter.json).
        ...
        // Verified against the real drifter's own AI tasks in that same JSON: wander uses
        // animation "standwalk", seekentity/fleeentity use "standrun", meleeattack uses
        // "standattack". These are the canonical (upright) names - the drifter's own
        // animationsRemapping table substitutes the crawl-state equivalent automatically when the
        // entity is in crawl state, so calling code doesn't need to know or care which state it's in.
        private const string IdleAnimCode = "idle";
        private const string WalkAnimCode = "standwalk";
        private const string RunAnimCode = "standrun";
        private const string AttackAnimCode = "standattack";
        ...
        public DrifterBehavior(ICoreClientAPI capi, string entityCode, Vec3d spawnPos)
        {
            this.capi = capi;
            ...
            capi.Logger.Notification($"remedyandruin: DrifterBehavior[{instanceId}] spawned={spawned} at {spawnPos}");
            ...
            capi.Logger.Notification($"remedyandruin: DrifterBehavior[{instanceId}] wander nodes ({nodeCount}): {string.Join(" | ", Array.ConvertAll(wanderNodes, n => n.ToString()))}");
        ...
        private void GoToCurrentWanderNode()
        {
            ...
            capi.Logger.Notification($"remedyandruin: DrifterBehavior[{instanceId}] pathfinding to wander node {nodeIndexForLog} at ({node.X:F1}, {node.Z:F1})");
            entity.MoveToSlow(node.X, node.Z, arrived =>
            {
                capi.Logger.Notification($"remedyandruin: DrifterBehavior[{instanceId}] wander node {nodeIndexForLog} callback: arrived={arrived}, state={state}");
                ...
                if (arrived)
                {
                    ...
                    capi.Logger.Notification($"remedyandruin: DrifterBehavior[{instanceId}] pausing {pauseTimer:F2}s at node {nodeIndexForLog}");
                }
                else
                {
        ...
        private void CheckForTeleport()
        {
            ...
            if (lastLoggedPos != null)
            {
                ...
                {
                    capi.Logger.Warning($"remedyandruin: DrifterBehavior[{instanceId}] TELEPORT jump of {moved:F2} blocks in one tick: {lastLoggedPos} -> {pos}");
                }
            }
        ...
        private void AdvanceWanderNode()
        {
            ...
            if (wanderNodeIndex >= wanderNodes.Length)
            {
                ...
                capi.Logger.Notification($"remedyandruin: DrifterBehavior[{instanceId}] wander nodes exhausted, beelining now");
            }
            else
            {
                state = State.Wandering;
        ...
        {
            // Horizontal-only, matching ShiverBehavior/BowtornBehavior's own attack-range check -
            // DistanceTo is a true 3D distance, so a vertical mismatch between the drifter's
            ...
            if (dist <= AttackRange)
            {
                ...
                capi.Logger.Notification($"remedyandruin: DrifterBehavior[{instanceId}] attacking at dist={dist:F2}, PlayOneShotAnimation returned {played}");
                return;
            }
